...
# read in tree
tree = read.tree('/Users/gdlab/Library/CloudStorage/Box-Box/DantasLabSriParuthiyil/DantasLab/Phage/plasmidome/260707_P4Revision/tree/all.bionj.asc.newick') # tips: 436

# reading in mash data
mash_data = data.frame(read_excel('/Users/gdlab/Library/CloudStorage/Box-Box/DantasLabSriParuthiyil/DantasLab/Phage/plasmidome/260707_P4Revision/tree/260623_finalized_144_assemblies_mash_checkm_quast_noBSI.xlsx'), stringsAsFactors = F)
mash_data <- mash_data %>%
...
pts = data.frame(read_excel('/Users/gdlab/Library/CloudStorage/Box-Box/pppp/data/260616_sip_updated_meta_v2.xlsx'), stringsAsFactors = F)
pts <- pts %>%
  filter(Cohort == "P4_cross_sectional")
...
X<-pts %>%
  group_by(Patient) %>%
  summarise(n_unique_abx_groups = n_distinct(abx_group_updated))

#Checking you only have one value of abx group
...
#tips_keep <- readLines("/Users/gdlab/Library/CloudStorage/Box-Box/DantasLabSriParuthiyil/DantasLab/Phage/plasmidome/250113_P4phagepipelinefinalizing/VIPtree/260503_succesfullymappedphages_V2.txt")
...
for(i in 1:nrow(ordered_df)){
  ordered_df$Mash[i] = mash_pt_data$Mash_call[mash_pt_data$Sample == ordered_df$Sample[i]]
...
ordered_df$Mash = gsub(" .*", " sp.", ordered_df$Mash)
...
plot_file <- file.path('/Users/gdlab/Library/CloudStorage/Box-Box/DantasLabSriParuthiyil/DantasLab/Phage/plasmidome/260707_P4Revision/', paste0(Sys.Date(), "abx_mash_phage.pdf"))
